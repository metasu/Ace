...
class AIAgent:
    ...
    def _max_tokens_param(self, value: int) -> dict:
        ...
        """
        if os.getenv("HERMES_FORCE_MAX_TOKENS_PROVIDERS", ""):
            _force_max_tokens = {
                item.strip().casefold()
                for item in os.getenv("HERMES_FORCE_MAX_TOKENS_PROVIDERS", "").split(",")
                if item.strip()
            }
            if str(getattr(self, "provider", "") or "").casefold() in _force_max_tokens:

...
def _format_connect_error(exc: BaseException) -> str:
    ...
    if isinstance(exc, asyncio.CancelledError):
        # Bare CancelledError from anyio cancel-scope machinery leaking through
        # asyncio.wait_for — render an actionable timeout diagnosis instead of
        # the opaque class name users saw in the gateway log.
        return _sanitize_error(
            "connection cancelled during startup — spawn/initialize did not "
            "complete within connect_timeout (slow network, first-run npx "
            "package download, or antivirus scanning; raise "
            "mcp_servers.<name>.connect_timeout)"
        )
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                self.session = None
                # If the cancellation hit mid-initial-connection (the per-server
                # asyncio.wait_for timeout in _discover_and_register_server
                # cancelling the transport through the MCP SDK's anyio
                # cancel-scope machinery) rather than an explicit shutdown,
                # flag a timeout error and wake the stranded start() waiter —
                # otherwise ``await self._ready`` hangs until the outer wait_for
                # and the caller surfaces a bare, opaque CancelledError.
                if not self._shutdown_event.is_set() and not self._ready.is_set():
                    self._error = asyncio.TimeoutError(
                        f"MCP server '{self.name}': connection cancelled during "
                        f"startup (spawn/initialize exceeded connect_timeout)"
                    )
                    self._ready.set()
                raise
            except Exception as exc:
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    # The outer budget adapts to the slowest configured server so a generous
    # per-server connect_timeout (e.g. 300s for a first-run npx package
    # download on a slow network) is not cancelled wholesale by a fixed cap.
    #
    ...
    try:
        outer_budget = max(
            120,
            max(
                (float(cfg.get("connect_timeout", _DEFAULT_CONNECT_TIMEOUT))
                 for cfg in new_servers.values()),
                default=float(_DEFAULT_CONNECT_TIMEOUT),
            ) + 30.0,
        )
    except (TypeError, ValueError):
        outer_budget = 120
    try:
        _run_on_mcp_loop(_discover_all, timeout=outer_budget)
    finally:
        if _was_interrupted:
            _set_interrupt(True)

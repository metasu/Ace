...
from agent.compact_request import apply_compact_request_if_needed
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Compact providers (e.g. Xiaoyi Chat Completions) time out on
                # Hermes' large system prompt, but MUST keep the full tool
                # catalog. Dropping tools made GPT-6 look like it could not
                # edit files. See agent/compact_request.py.
                apply_compact_request_if_needed(
                    api_kwargs, getattr(agent, "provider", None)
                )
                ...
                _use_streaming = True
                # Some OpenAI-compatible relays accept ``stream=true`` and return
                # HTTP 200 / ``text/event-stream`` but never emit an SSE data
                # frame.  They cannot be auto-detected as "stream unsupported"
                # because the request only ends in a read timeout.  Allow local
                # deployments to force those providers onto the reliable
                # non-streaming path without disabling browser-side streaming for
                # every other provider. Values are comma-separated provider IDs
                # and matched case-insensitively.
                _non_streaming_providers = {
                    item.strip().casefold()
                    for item in os.getenv("HERMES_NON_STREAMING_PROVIDERS", "").split(",")
                    if item.strip()
                }
                _provider_for_streaming = str(getattr(agent, "provider", "") or "").casefold()
                # Provider signaled "stream not supported" on a previous
                # attempt, or the deployment explicitly disabled its broken SSE
                # path — switch to non-streaming for the rest of this session.
                if (
                    getattr(agent, "_disable_streaming", False)
                    or _provider_for_streaming in _non_streaming_providers
                ):
                ...
                if retry_count >= max_retries:
                    # Some relays consistently accept the request and then hang.
                    # Rebuilding their local HTTP client only repeats the same
                    # upstream wait, so allow an explicit provider-scoped bypass
                    # to activate the configured fallback immediately.
                    _skip_primary_recovery_providers = {
                        item.strip().casefold()
                        for item in os.getenv("HERMES_SKIP_PRIMARY_RECOVERY_PROVIDERS", "").split(",")
                        if item.strip()
                    }
                    _skip_primary_recovery = (
                        str(getattr(agent, "provider", "") or "").casefold()
                        in _skip_primary_recovery_providers
                    )
                    # Before falling back, try rebuilding the primary
                    # client once for transient transport errors (stale
                    # connection pool, TCP reset).  Only attempted once
                    # per API call block unless this provider is configured
                    # to bypass that recovery.
                    if (
                        not _skip_primary_recovery

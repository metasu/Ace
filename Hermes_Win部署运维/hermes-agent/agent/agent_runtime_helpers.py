...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if "http_client" not in client_kwargs:
        _skip_custom_http_providers = {
            item.strip().casefold()
            for item in os.getenv("HERMES_SKIP_CUSTOM_HTTP_PROVIDERS", "").split(",")
            if item.strip()
        }
        if str(getattr(agent, "provider", "") or "").casefold() not in _skip_custom_http_providers:
            keepalive_http = agent._build_keepalive_http_client(

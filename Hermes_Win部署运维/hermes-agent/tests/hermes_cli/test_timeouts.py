...
def test_provider_timeout_matches_mixed_case_config_key(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    _write_config(
        tmp_path,
        """\
    ...
    assert get_provider_request_timeout("sudocx-gpt-5.6-sol", "gpt-5.6-sol") == 300.0
    assert get_provider_stale_timeout("sudocx-gpt-5.6-sol", "gpt-5.6-sol") is None

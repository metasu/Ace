...
import { mkdir, writeFile, unlink, readFile, rename } from 'fs/promises'; // Add unlink, readFile
...
export async function downloadFile(url, outputPath) {
    ...
    try {
        // axios's native timeout is reset once response headers are received and
        // does NOT protect against a stalled response body. Disable it and use
        // our own timers: 60s to get the first response headers and an overall
        // 150s deadline so a hung CDN connection fails fast while the tool can
        // still return the remote URL.
        ...
        // axios swallows the abort reason and just throws "canceled". Replace
        // it with a message that still identifies the URL, so the handler can
        // explain to the user that the remote URL was returned but the local

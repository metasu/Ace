...
export async function downloadFile(url, outputPath) {
    ...
    try {
        totalTimeout = setTimeout(() => {
            controller.abort(new Error(`Download total timeout exceeded for ${url}`));
        }, 150000);

        headerTimeout = setTimeout(() => {
            controller.abort(new Error(`Download header timeout: no response from ${url} for 60s`));
        }, 60000);
        ...
        if (Number.isFinite(declaredLength) && declaredLength >= 0 && stats.size !== declaredLength) {
            throw new Error(`Incomplete download: received ${stats.size} of ${declaredLength} bytes`);
        }
        if (stats.size === 0) {
        ...
        if (extension === '.png') {
            ...
            if (data.subarray(0, 8).toString('hex') !== signature || !data.subarray(-12).equals(Buffer.from(iend, 'hex'))) {
                throw new Error('Downloaded PNG is incomplete or invalid (missing PNG IEND trailer)');
            }
        }
        ...
        if (error?.name === 'CanceledError' || error?.constructor?.name === 'CanceledError' || error?.name === 'AbortError') {
            throw new Error(`Download aborted/timed out for ${url}`, { cause: error });
        }
        throw error;
    }
}
...
export async function saveImageToFile(imageBuffer, outputDir, prefix) {
    ...
    console.error(`[openapi-integrator-mcp] Image saved locally to: ${filePath}`);
    return filePath;
}
...
filename, uploadUrl, apiKey) {
    try {
        ...
        console.error(`[openapi-integrator-mcp] Attempting Cloudflare ImgBed upload to: ${uploadUrl} with filename: ${filename}`);
        ...
        if (response.status === 200 && Array.isArray(response.data) && response.data.length > 0 && response.data[0]?.src) {
            ...
            console.error(`[openapi-integrator-mcp] Cloudflare ImgBed upload successful: ${fullUrl}`);
            return fullUrl;
        }
        else {
...
export async function sendOneBotNotification(config, message) {
    ...
    console.error(`[OneBot Notification] Sending ${config.onebotMessageType} notification to target ${config.onebotTargetId} via ${config.onebotHttpUrl}...`);
    try {
        ...
        console.error('[OneBot Notification] Notification sent successfully.');
    }
    catch (error) {
...
export async function sendTelegramNotification(config, message) {
    ...
    console.error(`[Telegram Notification] Sending notification to Chat ID ${config.telegramChatId}...`);
    try {
        ...
        console.error('[Telegram Notification] Notification sent successfully.');
    }
    catch (error) {
...
taskType) {
    ...
    if (cloudflareUrl) {
        ...
        console.error(`[Notification] sendImageUploadNotification called with no cloudflareUrl and no localPath for ${filename}. This indicates a prior processing error.`);
    }
    console.error(`[Notification] Sending image ${taskType} success notification for ${filename}`); // Internal log in English
...
async function checkVideoStatus(requestId, apiKey, config) {
    ...
    console.error(`[SiliconFlow] Checking status for requestId: ${requestId} at ${url}`);
    try {
        ...
        console.error(`[SiliconFlow] Status for ${requestId}: ${response.data.status}`);
...
modelUsed, apiKey, config) {
    ...
    console.error(`[SiliconFlow] Starting background polling for video requestId: ${requestId}`);
    const poll = async () => {
        ...
        switch (statusResult.status) {
            case 'Succeed':
                console.error(`[SiliconFlow] Video generation succeeded for requestId: ${requestId}`);
                ...
                try {
                    ...
                    console.error(`[SiliconFlow] Downloading video from ${videoUrl} to ${localVideoPath}`);
                    await downloadFile(videoUrl, localVideoPath);
                    console.error(`[SiliconFlow] Video saved locally: ${localVideoPath}`);
                    let cfUploadMsg = "";
                    if (config.cfImgbedUploadUrl && config.cfImgbedApiKey) {
                        console.error(`[SiliconFlow] Attempting to upload video ${localVideoPath} to Cloudflare ImgBed...`);
...
config, taskType // To customize messages
    ...
    if (config.cfImgbedUploadUrl && config.cfImgbedApiKey && imageBuffer && localPath) {
        ...
        console.error(`[openapi-integrator-mcp BG] Skipping Cloudflare upload for ${taskNameEnglish} image ${filename} because local processing failed or buffer/path is missing.`);
...
export async function processImageGenerationInBackground(args, config, axiosInstance) {
    ...
    console.error('[openapi-integrator-mcp BG] Image generation request body:', JSON.stringify(requestBody, null, 2));
    let responseData;
    try {
    ...
    for (const item of responseData) {
        ...
        try {
            if (item.b64_json) {
                ...
                console.error(`[openapi-integrator-mcp BG] Image saved locally from b64_json to: ${localPath}`);
            }
                ...
                localPath = path.join(imagesOutputDir, filename);
                console.error(`[openapi-integrator-mcp BG] Downloading image from ${imageUrl} to ${localPath}`);
                await downloadFile(imageUrl, localPath);
                console.error(`[openapi-integrator-mcp BG] Image downloaded: ${localPath}`);
...
export async function processImageEditInBackground(args, config, axiosInstance) {
    ...
    try {
        if (isValidHttpUrl(args.image)) {
            ...
            console.error(`[openapi-integrator-mcp BG] Downloading image for editing from ${args.image} to ${tempImagePath}`); // Log in English
    ...
    console.error(`[openapi-integrator-mcp BG] Image Edit: Model used: ${modelToUseForEdit} (normalized to ${modelNameLowerEdit}). Added response_format: ${addedResponseFormat}`);
    let editedImageData;
    try {
        console.error(`[openapi-integrator-mcp BG] Sending image edit request with model ${modelToUseForEdit}.`);
    ...
    for (const item of editedImageData) {
        if (!item.b64_json) {
            console.error('[openapi-integrator-mcp BG] API edit response item missing b64_json data.'); // Log in English
        ...
        try {
            localPath = path.join(imagesOutputDir, filename);
            await fs.promises.writeFile(localPath, imageBuffer);
            console.error(`[openapi-integrator-mcp BG] Edited image saved locally to: ${localPath}`);
        }
        catch (err) {

...
async function pollPrediction(predictionId, apiKey, apiUrl) {
    ...
    while (Date.now() - startTime < MAX_POLL_TIME) {
        ...
        console.error(`[atlascloud-seedream] Polling prediction ${predictionId}: status=${status}`);
...
export async function handleToolCall({ toolName, args, axiosInstance, config }) {
    try {
        switch (toolName) {
            case 'edit_image': {
                ...
                if (!isValidHttpUrl(args.image)) {
                    ...
                    console.error(`[atlascloud-seedream] Uploading local image to AtlasCloud: ${args.image}`);
                    imageUrl = await uploadToAtlasCloud(args.image, apiKey, apiUrl);
                    console.error(`[atlascloud-seedream] Image uploaded, URL: ${imageUrl}`);
                }
                ...
                console.error(`[atlascloud-seedream] Submitting edit request to ${generateUrl}`);
                console.error(`[atlascloud-seedream] Request body: ${JSON.stringify(requestBody, null, 2)}`);
                ...
                console.error(`[atlascloud-seedream] Prediction ID: ${predictionId}, polling for result...`);
                ...
                for (const outputUrl of outputs) {
                    ...
                    try {
                        ...
                        console.error(`[atlascloud-seedream] Edited image saved to: ${localPath}`);
                    } catch (err) {
                        console.error(`[atlascloud-seedream] Error saving edited image: ${err.message}`);
        ...
        console.error(`[atlascloud-seedream] Error calling tool ${toolName}:`, error);
        ...
        if (axios.isAxiosError(error)) {
            console.error('[atlascloud-seedream] Axios error details:', {

...
async function pollPrediction(predictionId, apiKey, apiUrl) {
    ...
    while (Date.now() - startTime < MAX_POLL_TIME) {
        ...
        console.error(`[atlascloud-wan-pro] Polling prediction ${predictionId}: status=${status}`);
...
export async function handleToolCall({ toolName, args, axiosInstance, config }) {
    try {
            ...
            case 'edit_image': {
                ...
                for (const img of imageInputs) {
                    if (isValidHttpUrl(img)) {
                        ...
                        console.error(`[atlascloud-wan-pro] Uploading local image to AtlasCloud: ${img}`);
                        const uploadedUrl = await uploadToAtlasCloud(img, apiKey, apiUrl);
                        imageUrls.push(uploadedUrl);
                        console.error(`[atlascloud-wan-pro] Image uploaded, URL: ${uploadedUrl}`);
                    }
                }
                ...
                console.error(`[atlascloud-wan-pro] Submitting edit request to ${generateUrl}`);
                console.error(`[atlascloud-wan-pro] Request body: ${JSON.stringify(requestBody, null, 2)}`);
                ...
                console.error(`[atlascloud-wan-pro] Prediction ID: ${predictionId}, polling for result...`);
                ...
                for (const outputUrl of outputs) {
                    ...
                    try {
                        ...
                        console.error(`[atlascloud-wan-pro] Edited image saved to: ${localPath}`);
                    } catch (err) {
                        console.error(`[atlascloud-wan-pro] Error saving edited image: ${err.message}`);
        ...
        console.error(`[atlascloud-wan-pro] Error calling tool ${toolName}:`, error);
        ...
        if (axios.isAxiosError(error)) {
            console.error('[atlascloud-wan-pro] Axios error details:', {

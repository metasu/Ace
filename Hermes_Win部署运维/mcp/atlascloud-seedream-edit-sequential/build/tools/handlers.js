...
async function pollPrediction(predictionId, apiKey, apiUrl) {
    ...
    while (Date.now() - startTime < MAX_POLL_TIME) {
        ...
        console.error(`[atlascloud-seedream-seq] Polling prediction ${predictionId}: status=${status}`);
...
export async function handleToolCall({ toolName, args, axiosInstance, config }) {
    try {
        switch (toolName) {
            case 'edit_image': {
                ...
                if (!isValidHttpUrl(args.image)) {
                    try {
                        await fs.promises.access(args.image, fs.constants.R_OK);
                    } catch (accessError) {
                        throw new McpError(ErrorCode.InvalidParams, `Cannot access local image file: ${args.image}`);
                    }
                    console.error(`[atlascloud-seedream-seq] Uploading local image to AtlasCloud: ${args.image}`);
                    imageUrl = await uploadToAtlasCloud(args.image, apiKey, apiUrl);
                    console.error(`[atlascloud-seedream-seq] Image uploaded, URL: ${imageUrl}`);
                }
                ...
                if (args.max_images !== undefined) {
                    requestBody.max_images = args.max_images;
                }
                ...
                console.error(`[atlascloud-seedream-seq] Submitting edit request to ${generateUrl}`);
                console.error(`[atlascloud-seedream-seq] Request body: ${JSON.stringify(requestBody, null, 2)}`);
                ...
                console.error(`[atlascloud-seedream-seq] Prediction ID: ${predictionId}, polling for result...`);
                ...
                for (const outputUrl of outputs) {
                    ...
                    const filename = `edited_image_${randomUUID()}.${ext}`;
                    try {
                        localPath = path.join(imagesOutputDir, filename);
                        await downloadFile(outputUrl, localPath);
                        console.error(`[atlascloud-seedream-seq] Edited image saved to: ${localPath}`);
                    } catch (err) {
                        console.error(`[atlascloud-seedream-seq] Error saving edited image: ${err.message}`);
        ...
        console.error(`[atlascloud-seedream-seq] Error calling tool ${toolName}:`, error);
        ...
        if (axios.isAxiosError(error)) {
            console.error('[atlascloud-seedream-seq] Axios error details:', {

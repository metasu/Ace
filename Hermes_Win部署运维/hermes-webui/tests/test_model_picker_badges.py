...
def test_minimal_catalog_preserves_custom_provider_failover_badges(monkeypatch):
    monkeypatch.setattr(
        config,
        "cfg",
        {
            "model": {"provider": "cc-vibe-gpt", "default": "gpt-5.6-sol"},
            "providers": {
                "cc-vibe-gpt": {"base_url": "https://cc-vibe.com/v1"},
                "kuaipao": {"base_url": "https://kuaipao.ai/v1"},
                "atlascloud": {"base_url": "https://api.atlascloud.ai/v1"},
            },
            "fallback_providers": [
                {"provider": "kuaipao", "model": "gpt-5.6-sol"},
                {"provider": "atlascloud", "model": "xai/grok-4.3"},
            ],
        },
    )
    ...
    assert badges["@cc-vibe-gpt:gpt-5.6-sol"]["role"] == "primary"
    assert badges["@kuaipao:gpt-5.6-sol"]["label"] == "Fallback 1"
    assert badges["@atlascloud:xai/grok-4.3"]["label"] == "Fallback 2"

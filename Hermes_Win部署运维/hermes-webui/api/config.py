...
def _merge_session_toolsets(base_toolsets, override, cfg=None):
    """Resolve a per-session ``enabled_toolsets`` override against the profile
    defaults.

    The session override is stored as a raw toolset-name allowlist. A list
    naming ONLY configured MCP servers — exactly what the composer MCP
    checkboxes write — is additive on top of ``base_toolsets``: selecting
    ``github`` must not strip the whole native toolset set. A list containing
    at least one non-MCP entry is a genuine restriction and is returned
    unchanged. ``mcp-<server>`` spellings count as MCP entries too.
    """
    if cfg is None:
        try:
            cfg = get_config() or {}
            ...
            cfg = {}
    mcp_names = {str(name) for name in (cfg.get("mcp_servers") or {})}

    def _is_mcp_entry(name):
        name = str(name)
        return name in mcp_names or (name.startswith("mcp-") and name[4:] in mcp_names)

    entries = list(override or [])
    if entries and all(_is_mcp_entry(entry) for entry in entries):
        merged = list(base_toolsets or [])
        seen = set(merged)
...
def _config_model_allowlist_label(model_id: object) -> str:
    """Display label for a ``providers.<name>.models`` allowlist entry.

    The option value keeps the full routing id (``xai/grok-4.3``) because that
    is what the provider expects, but the picker shows only the model name
    (``grok-4.3``). URI-scheme ids are left untouched — their slashes are path
    separators, not a vendor prefix.
    """
    mid = str(model_id or "").strip()
        ...
        for pid in sorted(detected_providers):
            ...
            if isinstance(provider_cfg, dict) and "models" in provider_cfg:
                ...
                if isinstance(cfg_models, dict):
                    raw_models = [
                        {"id": key, "label": _config_model_allowlist_label(key)}
                        for key in cfg_models.keys()
                    ]
                            ...
                            raw_models.append(
                                {
                                    ...
                                    or _config_model_allowlist_label(model_id),
                                }
                            )
                        elif item:
                            raw_models.append(
                                {"id": item, "label": _config_model_allowlist_label(item)}
                            )
            if not raw_models:
            ...
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    if (
                        ...
                        if isinstance(cfg_models, dict):
                            raw_models = [
                                {"id": k, "label": _config_model_allowlist_label(k)}
                                for k in cfg_models.keys()
                            ]
                        elif isinstance(cfg_models, list):
                            raw_models = [
                                {
                                    "id": k["id"] if isinstance(k, dict) else k,
                                    "label": (
                                        k.get("label")
                                        or _config_model_allowlist_label(k["id"])
                                    )
                                    if isinstance(k, dict)
                                    else _config_model_allowlist_label(k),
                                }
                                for k in cfg_models
                            ]

...
# MCP connections are process-global, while WebUI starts one worker per turn.
# Keep discovery idempotent at the WebUI boundary too: a failed or slow MCP
# server must not make every message repeat the full discovery wait.
_MCP_DISCOVERY_LOCK = threading.Lock()
_MCP_DISCOVERY_FINGERPRINT = None
_MCP_DISCOVERY_LAST_ATTEMPT = 0.0
_MCP_DISCOVERY_RETRY_AFTER = 60.0
    ...
    try:
        ...
        with profiles_api.profile_env_for_background_worker(s, "background title", logger_override=logger):
            ...
            if agent and not aux_title_configured:
                # The active agent already uses the selected primary provider.
                # Do not fall through to auxiliary auto-routing here: when no
                # auxiliary credentials exist, that path probes OpenRouter and
                # adds avoidable latency after every first turn. Local fallback
                # below remains available and explicit auxiliary configuration
                # keeps the existing auxiliary behavior.
...
def _run_background_title_refresh(session_id: str, user_text: str, assistant_text: str, current_title: str, put_event, agent=None):
    ...
    try:
        ...
        with profiles_api.profile_env_for_background_worker(s, "background title", logger_override=logger):
            aux_title_configured = _aux_title_configured()
            if agent and not aux_title_configured:
                # Keep refreshes on the selected primary provider. An
                # unconfigured auxiliary auto-route must not probe OpenRouter.
                next_title, llm_status, raw_preview = _generate_llm_session_title_for_agent(agent, user_text, assistant_text)
            else:
                next_title, llm_status, raw_preview = _generate_llm_session_title_via_aux(user_text, assistant_text)
                if not next_title and agent and llm_status in ('llm_error_aux', 'llm_invalid_aux'):
...
def _run_agent_streaming(
    ...
    *,
    ...
    try:
        ...
        # ── MCP Server Discovery (lazy, configuration-aware, process-wide) ──
        # MUST run AFTER the per-profile HERMES_HOME mutation above. MCP
        # connections are process-global, so repeating discovery for every
        # WebUI turn is unnecessary and can block on a broken stdio server.
        # A changed MCP config invalidates the gate; failed discovery is cooled
        # down briefly so all existing MCP entries remain available without
        # turning transient MCP failures into per-message latency.
        try:
            from tools.mcp_tool import discover_mcp_tools
            _mcp_cfg = (get_config() or {}).get('mcp_servers') or {}
            _mcp_fingerprint = json.dumps(
                _mcp_cfg, sort_keys=True, ensure_ascii=True, default=str,
            )
            _mcp_now = time.monotonic()
            global _MCP_DISCOVERY_FINGERPRINT, _MCP_DISCOVERY_LAST_ATTEMPT
            with _MCP_DISCOVERY_LOCK:
                _should_discover = (
                    _mcp_fingerprint != _MCP_DISCOVERY_FINGERPRINT
                    or (_mcp_now - _MCP_DISCOVERY_LAST_ATTEMPT) >= _MCP_DISCOVERY_RETRY_AFTER
                )
                if _should_discover:
                    _MCP_DISCOVERY_LAST_ATTEMPT = _mcp_now
                    discover_mcp_tools()
                    _MCP_DISCOVERY_FINGERPRINT = _mcp_fingerprint
        except Exception:
            logger.debug("MCP discovery unavailable; continuing without blocking the turn", exc_info=True)
        ...
        try:
            ...
            from api.config import _resolve_cli_toolsets, _merge_session_toolsets
            ...
            try:
                ...
                if _session_path.exists():
                    ...
                    if _override:
                        # An override naming ONLY configured MCP servers (the
                        # composer MCP checkbox flow) is additive on top of the
                        # profile defaults — restricting to e.g. ["github"]
                        # would strip every native toolset. Overrides with any
                        # non-MCP entry keep the restrict semantics unchanged.
                        _toolsets = _merge_session_toolsets(_toolsets, _override, _cfg)

...
// Legacy "<providerId>/<model>" values (e.g. atlascloud-grok-4.3/xai/grok-4.3)
// were persisted by older WebUI builds and by stale localStorage entries. The
// provider segment is not part of the model ID, so strip it when it names one
// of the dropdown's own provider groups — otherwise the value never resolves
// and gets injected as a duplicate session-scoped row.
function _stripProviderSegmentFromModelId(modelId, sel, preferredProviderId){
  const raw=String(modelId||'');
  if(!raw||raw.startsWith('@')||!raw.includes('/')||raw.includes('://')) return raw;
  const head=raw.slice(0,raw.indexOf('/')).toLowerCase();
  const rest=raw.slice(raw.indexOf('/')+1);
  if(!head||!rest) return raw;
  const known=new Set();
  if(preferredProviderId) known.add(String(preferredProviderId).toLowerCase());
  if(sel){
    for(const o of Array.from(sel.options||[])){
      const pid=_getOptionProviderId(o);
      if(pid) known.add(String(pid).toLowerCase());
    }
  }
  for(const pid of Object.values(window._configuredModelBadges||{})){
    if(pid&&pid.provider) known.add(String(pid.provider).toLowerCase());
  }
...
function _findModelInDropdown(modelId, sel, preferredProviderId){
  ...
  if(!opts.includes(modelId)){
    const canonical=_stripProviderSegmentFromModelId(modelId,sel,preferredProviderId);
    if(canonical!==modelId&&opts.includes(canonical)) modelId=canonical;
  }
...
function renderModelDropdown(){
  ...
  const _buildModelRow=(m,sel,withProviderChip)=>{
    ...
    const _displayName=m.id?getModelLabel(m.id):'';
    ...
    row.innerHTML=`<div class="model-opt-top"><span class="model-opt-name">${esc(_displayName)}</span>${badgeHtml}${providerChip}</div><span class="model-opt-id">${esc(m.id)}</span>`;
  ...
  const _filterModels=(term)=>{
    ...
    if(configuredModels.length){
      ...
      for(const m of configuredModels){
        ...
        row.className='model-opt'+(m.value===sel.value?' active':'');
        let badgeLabel = '';
        let modelName = m.id?getModelLabel(m.id):m.name;
        if (m.badge) {
...
function _applyToolsetsChip(toolsets) {
  ...
  if (hasCustom) {
    ...
    const mcpOnly = toolsets.every(function(name) {
      const n = String(name || '');
      return catalogSet.has(n) || n.indexOf('mcp-') === 0;
    });
    const shown = (mcpOnly && catalogSet.size)
      ? ('defaults + ' + toolsets.join(', '))
      : toolsets.join(', ');
    label.textContent = shown + stagedSuffix;
    chip.classList.add('has-custom');
    chip.title = t('session_toolsets') + ': ' + shown + stagedSuffix;
  } else {

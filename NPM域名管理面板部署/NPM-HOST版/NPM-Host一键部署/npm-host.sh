#!/usr/bin/env bash
# npm-host.sh - v4.6.9（NPM Host Network）
# 组件：Nginx Proxy Manager（管理面板） + MariaDB
# 特性：host 网络模式；生成 .env；一键 up/down/logs/status；适配 Docker Compose v2
# ------------------------------------------------------------------------------

set -u -o pipefail
[[ "${DEBUG:-0}" == "1" ]] && set -x

ROOT_DIR="$(dirname "$(readlink -f "${0:-$PWD/npm-host.sh}")")"
...
services:
  ...
  npm-db:
    ...
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "127.0.0.1", "-P", "${MYSQL_PORT}"]
      interval: 10s
      timeout: 5s
      retries: 10
YAML
...
do_up(){
  ...
  echo
  echo "================= 🚀 部署完成（NPM Host 网络）🚀 ================="
  echo "NPM 管理面板: http://${PUBLIC_IP}:81"
  echo "HTTP 代理端口: 80    HTTPS 代理端口: 443"
  echo "MariaDB 端口: ${MYSQL_PORT}（仅本机 127.0.0.1）"
  echo "================================================================"
  compose ps || true
}
...
menu(){
  clear
  echo -e "${G}=== NPM 一键部署 v4.6.9（Host 网络）===${NC}"
  echo "1) 安装/更新（up）"
  echo "2) 查看状态（status）"
  echo "3) 实时日志（logs）"
  echo "4) 停止服务（down）"

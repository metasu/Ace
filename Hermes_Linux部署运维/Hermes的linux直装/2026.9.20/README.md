...
| `HERMES_XIAOYI_ASTRA_KEY` | xiaoyiapi.xyz | gpt-6-astra（默认 provider）|
| `HERMES_XIAOYI_FABLE_KEY` | xiaoyiapi.xyz | claude-fable-5-1 |
...
通过 `hermesctl.sh` 数字菜单切换（默认 `xiaoyi-gpt-6-astra → gpt-6-astra`）：

| 菜单 | Provider | 模型 |
|---|---|---|
| `[17]` | xiaoyi-gpt-6-astra | gpt-6-astra（当前生产默认）|
| `[18]` | xiaoyi-claude-fable-5-1 | claude-fable-5-1 |
...
1. xiaoyi-claude-fable-5-1 / claude-fable-5-1
2. atlascloud-grok-4.3 / xai/grok-4.3
3. atlascloud-grok-4.6 / xai/grok-4.6

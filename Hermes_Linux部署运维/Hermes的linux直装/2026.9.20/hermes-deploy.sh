...
step_env(){
  ...
  if [[ -f "${HERMES_DATA}/.env" ]]; then
    ...
    for key in HERMES_XIAOYI_ASTRA_KEY HERMES_XIAOYI_FABLE_KEY; do
...
HERMES_XIAOYI_FABLE_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
...
# 回退链中保留 Atlas Grok 4.3 与 Atlas Grok 4.6 各一条（新顺序：4.3 在前）。
fallback_entry = """  - provider: xiaoyi-claude-fable-5-1
    model: claude-fable-5-1
    key_env: HERMES_XIAOYI_FABLE_KEY
...
if not re.search(r"^  xiaoyi-claude-fable-5-1:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  xiaoyi-claude-fable-5-1:\n"
...
控制脚本：
  ...
  # switch 示例: xiaoyi-gpt-6-astra（默认）| xiaoyi-claude-fable-5-1 | atlascloud-grok-4.3 | atlascloud-grok-4.6

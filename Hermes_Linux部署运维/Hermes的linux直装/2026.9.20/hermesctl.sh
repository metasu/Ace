...
switch_provider(){
  ...
  case "$provider" in
    xiaoyi-claude-fable-5-1|xiaoyi-fable-5-1|fable-5-1|fable)
...
切换: switch <provider> [--model m] [--url u]
      provider: xiaoyi-gpt-6-astra | xiaoyi-claude-fable-5-1 |
...
show_menu(){
  ...
  echo "  [18] xiaoyi-claude-fable-5-1    → claude-fable-5-1"
...
menu_loop(){
  ...
  while true; do
    ...
    case "$choice" in
      ...
      18) switch_provider xiaoyi-claude-fable-5-1; pause ;;

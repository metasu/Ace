...
step_env(){
  ...
  if [[ -f "${HERMES_DATA}/.env" ]]; then
    ...
    for key in HERMES_XIAOYI_OPUS_KEY HERMES_XIAOYI_SOL_KEY; do
...
HERMES_XIAOYI_SOL_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
...
# 若当前主模型指向未知 provider，则回退到默认 xiaoyi-claude-opus-5。
m = re.search(r'^  provider:\s*["\']?([^"\'\n]+)', t2, re.M)
known = {
    "xiaoyi",
    "xiaoyi-claude-opus-5",
    "xiaoyi-gpt-5-6-sol",
    "atlascloud",
    "atlascloud-grok-4.3",
    "atlascloud-grok-4.6",
...
if m and m.group(1).strip() not in known:
    t2 = re.sub(r'^  default:.*$', '  default: "claude-opus-5"', t2, count=1, flags=re.M)
    t2 = re.sub(r'^  provider:.*$', '  provider: "xiaoyi-claude-opus-5"', t2, count=1, flags=re.M)
...
fallback_entry = """  - provider: xiaoyi-gpt-5-6-sol
    model: gpt-5.6-sol
    key_env: HERMES_XIAOYI_SOL_KEY
...
if not re.search(r"^  xiaoyi-claude-opus-5:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  xiaoyi-claude-opus-5:\n"
...
if not re.search(r"^  xiaoyi-gpt-5-6-sol:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  xiaoyi-gpt-5-6-sol:\n"
...
控制脚本：
  /opt/hermes/hermesctl.sh          # 数字菜单（模型切换 [17]–[20] 在最后；默认 [17] xiaoyi-gpt）
  /opt/hermes/hermesctl.sh {start|stop|restart|status|mcp|switch|test}
  # switch 示例: xiaoyi-gpt（默认）| atlascloud-grok-4.3 | atlascloud-grok-4.6

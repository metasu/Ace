...
switch_provider(){
  ...
  case "$provider" in
    xiaoyi|xiaoyi-claude-opus-5|xiaoyi-opus-5|opus-5)
      provider="xiaoyi-claude-opus-5"
      model="${model:-claude-opus-5}"
      url="${url:-https://xiaoyiapi.xyz/v1}"
      ;;
    xiaoyi-gpt-5.6-sol|xiaoyi-gpt-5-6-sol|sol)
...
切换: switch <provider> [--model m] [--url u]
      provider: xiaoyi-claude-opus-5 | xiaoyi-gpt-5-6-sol |
                atlascloud-grok-4.3 | atlascloud-grok-4.6
      （默认 xiaoyi-claude-opus-5）
...
show_menu(){
  ...
  echo
  echo -e "${G}  切换主模型（默认: xiaoyi-claude-opus-5 → claude-opus-5）${NC}"
  echo "  [17] xiaoyi-claude-opus-5       → claude-opus-5  (默认)"
  echo "  [18] xiaoyi-gpt-5-6-sol         → gpt-5.6-sol"
...
menu_loop(){
  ...
  while true; do
    ...
    case "$choice" in
      ...
      17) switch_provider xiaoyi-claude-opus-5; pause ;;
      18) switch_provider xiaoyi-gpt-5-6-sol; pause ;;

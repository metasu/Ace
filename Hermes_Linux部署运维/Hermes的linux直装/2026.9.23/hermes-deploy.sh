...
step_env(){
  ...
  if [[ -f "${HERMES_DATA}/.env" ]]; then
    ...
    for key in XIAOYI_GROK_IMAGE_KEY MCP_ATLASCLOUD_KEY XIAOYI_GROK_IMAGE_API_URL MCP_ATLASCLOUD_API_URL; do
      if ! grep -q "^${key}=" "${HERMES_DATA}/.env"; then
        case "$key" in
          XIAOYI_GROK_IMAGE_API_URL)   printf '%s=https://xiaoyiapi.xyz/v1\n' "$key" >> "${HERMES_DATA}/.env" ;;
...
XIAOYI_GROK_IMAGE_API_URL=https://xiaoyiapi.xyz/v1
...
step_inject_mcp(){
  ...
  local xiaoyi_image_url="${XIAOYI_GROK_IMAGE_API_URL:-https://xiaoyiapi.xyz/v1}"

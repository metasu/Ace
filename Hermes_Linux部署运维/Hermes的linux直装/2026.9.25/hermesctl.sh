...
cmd_webui_patch(){
  ...
  if [[ -f "$media_py" ]]; then
    say "应用 MEDIA: 正则修复补丁（排除 markdown ** 加粗符号）"
    python3 "$media_py" "$HERMES_WEBUI" || die "media-regex-patch 失败"
  fi
  ...
  if [[ -f "$nullchunk_py" ]]; then
    say "应用流式 null-chunk 补丁（跳过上游 `data: null` SSE 帧）"
    python3 "$nullchunk_py" "$HERMES_DIR" || die "stream-null-chunk-patch 失败"
  fi
  # 注入 MCP 配置（顺带）
  inject_mcp_config
  check_webui_mobile_mcp_entry
...
switch_provider(){
  ...
  case "$provider" in
    xiaoyi-claude-opus-5-5|xiaoyi-opus-5-5|opus-5-5|opus)
      provider="xiaoyi-claude-opus-5-5"
      model="${model:-claude-opus-5-5}"
      url="${url:-https://xiaoyiapi.xyz/v1}"
      ;;
    xiaoyi|xiaoyi-gpt-6-astra|xiaoyi-gpt-6|astra)
...
try:
    ...
    req.add_header('User-Agent', (m.get('default_headers') or {}).get('User-Agent') or os.environ.get('HERMES_USER_AGENT', 'Mozilla/5.0'))
    if key:
...
cmd_mcp(){
  ...
  echo
  if [[ -f "${mcp_dir}/xiaoyi-grok-image/build/index.js" ]]; then
    echo "--- hermes mcp test xiaoyi-grok-image ---"
    "$HERMES_BIN" mcp test xiaoyi-grok-image 2>&1 || true
  fi
}
...
cmd_mcp_export_patches(){
  ...
  local patch_dir="${HERMES_DIR}/mcp-patches"
  local servers=(
    xiaoyi-grok-image atlascloud-seedream-v5-pro atlascloud-seedream-edit
...
切换: switch <provider> [--model m] [--url u]
      provider: xiaoyi-gpt-6-astra | xiaoyi-claude-opus-5-5 |
                atlascloud-grok-4.3 | atlascloud-grok-4.6
      （默认 xiaoyi-gpt-6-astra）
...
show_menu(){
  ...
  echo
  echo -e "${G}  切换主模型（默认: xiaoyi-gpt-6-astra → gpt-6-astra）${NC}"
  echo "  [17] xiaoyi-gpt-6-astra         → gpt-6-astra  (默认)"
  echo "  [18] xiaoyi-claude-opus-5-5     → claude-opus-5-5"
  echo "  [19] atlascloud-grok-4.3        → xai/grok-4.3  (同 atlas API)"
  echo "  [20] atlascloud-grok-4.6        → xai/grok-4.6  (复用 atlas key)"
  echo
  echo "  [0]  退出"
  echo
}
...
menu_loop(){
  ...
  while true; do
    show_menu
    read -r -p "请选择 [0-20]: " choice || choice="0"
    choice="${choice// /}"
    echo
    case "$choice" in
      1)  cmd_start; pause ;;
      ...
      17) switch_provider xiaoyi-gpt-6-astra; pause ;;
      18) switch_provider xiaoyi-claude-opus-5-5; pause ;;
      19) switch_provider atlascloud-grok-4.3; pause ;;
      20) switch_provider atlascloud-grok-4.6; pause ;;

...
#   patches/fix_compact_keep_tools.py       — Xiaoyi 瘦请求但不剥 tools
#   patches/fix_stream_null_chunk.py        — 跳过上游 `data: null` SSE 帧
...
step_env(){
  ...
  if [[ -f "${HERMES_DATA}/.env" ]]; then
    ...
    if ! grep -q '^HERMES_ATLASCLOUD_KEY=' "${HERMES_DATA}/.env"; then
      echo "HERMES_ATLASCLOUD_KEY=apikey-RRRRRRRRRRRRRRRRRRRRRRRRRR" >> "${HERMES_DATA}/.env"
      wrn "已补齐 HERMES_ATLASCLOUD_KEY（占位符，请替换为真实密钥）"
    fi
    ...
    if ! grep -q '^HERMES_WEBUI_PASSWORD=' "${HERMES_DATA}/.env"; then
      printf '\n# ===== WebUI 登录密码（公网反代必开）=====\nHERMES_WEBUI_PASSWORD=RRRRRRRRRR\n' >> "${HERMES_DATA}/.env"
      wrn "已补齐 HERMES_WEBUI_PASSWORD（占位符，公网务必修改）"
    fi
    for key in HERMES_XIAOYI_ASTRA_KEY HERMES_XIAOYI_OPUS_KEY; do
      if ! grep -q "^${key}=" "${HERMES_DATA}/.env"; then
        printf '%s=\n' "$key" >> "${HERMES_DATA}/.env"
        wrn "已补齐 ${key}，请在 .env 中填入有效密钥"
      fi
    done
    for key in XIAOYI_GROK_IMAGE_KEY MCP_ATLASCLOUD_KEY XIAOYI_GROK_IMAGE_API_URL MCP_ATLASCLOUD_API_URL; do
      if ! grep -q "^${key}=" "${HERMES_DATA}/.env"; then
        case "$key" in
          XIAOYI_GROK_IMAGE_API_URL)   printf '%s=https://image.xiaoyiapi.xyz/v1\n' "$key" >> "${HERMES_DATA}/.env" ;;
...
HERMES_ATLASCLOUD_KEY=apikey-RRRRRRRRRRRRRRRRRRRRRRRRRR
HERMES_XIAOYI_ASTRA_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
HERMES_XIAOYI_OPUS_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR

# ===== MCP 服务配置（hermesctl.sh / hermes-deploy.sh 读取，不写入 config.yaml）=====
XIAOYI_GROK_IMAGE_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
XIAOYI_GROK_IMAGE_API_URL=https://image.xiaoyiapi.xyz/v1
MCP_ATLASCLOUD_KEY=apikey-RRRRRRRRRRRRRRRRRRRRRRRRRR
...
# ===== WebUI 登录密码（公网反代必开）=====
# 默认初始密码占位符；上线后请立即修改并 systemctl restart hermes-webui
HERMES_WEBUI_PASSWORD=RRRRRRRRRR
EOF
  chmod 600 "${HERMES_DATA}/.env"
  ok ".env 已生成（密钥为占位符，请替换为真实密钥后复核）"
  echo "    API_SERVER_KEY=${HERMES_API_KEY}"
  echo "    WebUI 初始密码: 见 .env HERMES_WEBUI_PASSWORD（公网务必修改）"
}
...
step_config(){
  ...
  if [[ -f "${HERMES_DATA}/config.yaml" ]]; then
    ...
    # 统一渠道 provider，并补齐 Atlas Grok 4.3 provider 与回退保护。
    python3 - <<'PY'
...
t2 = re.sub(r"^  xiaoyi-claude-fable-5-1:\n(?:    .*\n)*", "", t2, flags=re.M)
t2 = t2.replace("providers:\n", "providers:\n" + block, 1)
# 清理 atlascloud provider 中的 gpt-5.6-luna 残留（仅保留 base_url + key_env）
t2 = re.sub(r'^(  atlascloud:\n    base_url: [^\n]+\n    key_env: [^\n]+\n)    model: [^\n]*\n(?:    models:\n(?:      [^\n]*\n)*)?', r'\1', t2, flags=re.M)
# fable → opus 更名迁移：旧主模型仍指向 fable 时直接改到 opus，不回退 astra。
t2 = re.sub(r'^  provider:\s*["\']?xiaoyi-claude-fable-5-1["\']?\s*$', '  provider: "xiaoyi-claude-opus-5-5"', t2, count=1, flags=re.M)
t2 = re.sub(r'^  default:\s*["\']?claude-fable-5-1["\']?\s*$', '  default: "claude-opus-5-5"', t2, count=1, flags=re.M)
# 若当前主模型指向未知 provider，则回退到默认 xiaoyi-gpt-6-astra。
...
if m and m.group(1).strip() not in known:
    t2 = re.sub(r'^  default:.*$', '  default: "gpt-6-astra"', t2, count=1, flags=re.M)
    t2 = re.sub(r'^  provider:.*$', '  provider: "xiaoyi-gpt-6-astra"', t2, count=1, flags=re.M)
    t2 = re.sub(r'^  base_url:.*$', '  base_url: "https://xiaoyiapi.xyz/v1"', t2, count=1, flags=re.M)
# 回退链托管条目：xiaoyi opus → xiaoyi astra → Atlas Grok 4.3 → 4.6（重建前去重，保持幂等）。
fallback_entry = """  - provider: xiaoyi-claude-opus-5-5
    model: claude-opus-5-5
    key_env: HERMES_XIAOYI_OPUS_KEY
    base_url: https://xiaoyiapi.xyz/v1
  - provider: xiaoyi-gpt-6-astra
    model: gpt-6-astra
    key_env: HERMES_XIAOYI_ASTRA_KEY
    base_url: https://xiaoyiapi.xyz/v1
  - provider: atlascloud-grok-4.3
    model: xai/grok-4.3
    key_env: HERMES_ATLASCLOUD_KEY
    base_url: https://api.atlascloud.ai/v1
  - provider: atlascloud-grok-4.6
    model: xai/grok-4.6
    key_env: HERMES_ATLASCLOUD_KEY
    base_url: https://api.atlascloud.ai/v1
"""
t2 = re.sub(
    r"^ *- provider: (?:xiaoyi-claude-fable-5-1|xiaoyi-claude-opus-5-5|xiaoyi-gpt-6-astra|atlascloud(?:-grok-4\.[36])?)\n"
    r" +model: [^\n]+\n +key_env: [^\n]+\n(?: +base_url: [^\n]+\n)?(?:\n)*",
    "",
    t2,
    flags=re.M,
...
if "fallback_providers:\n" in t2:
    t2 = t2.replace("fallback_providers:\n", "fallback_providers:\n" + fallback_entry, 1)
# 补齐 xiaoyi 独立渠道（与 atlascloud-grok-4.3/4.6 同级拆分，各自独立 key）。
if not re.search(r"^  xiaoyi-claude-opus-5-5:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  xiaoyi-claude-opus-5-5:\n"
...
if not re.search(r"^  xiaoyi-gpt-6-astra:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  xiaoyi-gpt-6-astra:\n"
...
if not re.search(r"^  atlascloud-grok-4\.6:\n", t2, flags=re.M):
    t2 = t2.replace(
        "providers:\n",
        "providers:\n"
        "  atlascloud-grok-4.6:\n"
...
for provider_name, model_id in (
    ...
    ("xiaoyi-claude-opus-5-5", "claude-opus-5-5"),
    ("atlascloud-grok-4.3", "xai/grok-4.3"),
    ("atlascloud-grok-4.6", "xai/grok-4.6"),
):
    provider_block = re.search(rf"^  {re.escape(provider_name)}:\n(?:    .*\n)*", t2, flags=re.M)
    if provider_block and not re.search(r"^    models:", provider_block.group(), flags=re.M):
        t2 = t2[:provider_block.end()] + f"    models:\n      - {model_id}\n" + t2[provider_block.end():]
p.write_text(t2, encoding="utf-8")
print("normalized providers and atlascloud-grok-4.3 fallback")
PY
...
fallback_providers:
  - provider: xiaoyi-claude-opus-5-5
    model: claude-opus-5-5
    key_env: HERMES_XIAOYI_OPUS_KEY
    base_url: https://xiaoyiapi.xyz/v1
  - provider: xiaoyi-gpt-6-astra
...
# 单次回复的最大输出 token 上限。xiaoyi 的 Claude Opus 渠道要求非流式请求必须携带
# max_tokens，缺省会报 HTTP 400 "not found 'max_tokens'"。
max_tokens: 64000
...
step_inject_mcp(){
  ...
  local xiaoyi_image_key="${XIAOYI_GROK_IMAGE_KEY:-}"
  local xiaoyi_image_url="${XIAOYI_GROK_IMAGE_API_URL:-https://image.xiaoyiapi.xyz/v1}"
...
mcp_servers:
  xiaoyi-grok-image:
    command: node
    args:
      - ${root}/mcp/xiaoyi-grok-image/build/index.js
    timeout: 660
    env:
      API_KEY: ${xiaoyi_image_key}
      API_URL: ${xiaoyi_image_url}
...
EOF
  ...
  if [[ -n "$zip_path" && -f "$zip_path" && "${MCP_FROM_UPSTREAM:-0}" != "1" && "${FORCE_MCP_REBUILD:-0}" != "1" ]]; then
    ...
    if [[ -d "${ztmp}/xiaoyi-grok-image" ]]; then
      cp -a "${ztmp}/." "$HERMES_MCP/"
    else
      local sub
      sub="$(find "$ztmp" -maxdepth 2 -type d -name xiaoyi-grok-image | head -1)"
      [[ -n "$sub" ]] || die "zip 中无 xiaoyi-grok-image"
...
step_webui_mobile_patch(){
  ...
  local media_py="${HERMES_DIR}/patches/fix_media_regex_asterisk.py"
  if [[ -f "$media_py" ]]; then
    python3 "$media_py" "$HERMES_WEBUI" || wrn "MEDIA: 正则修复补丁执行失败（可稍后 hermesctl.sh webui-patch）"
  fi
  local swr_py="${HERMES_DIR}/patches/fix_session_visit_models_swr.py"
  if [[ -f "$swr_py" ]]; then
  ...
  local nullchunk_py="${HERMES_DIR}/patches/fix_stream_null_chunk.py"
  if [[ -f "$nullchunk_py" ]]; then
    python3 "$nullchunk_py" "$HERMES_DIR" || wrn "null-chunk 补丁执行失败（可稍后 hermesctl.sh webui-patch）"
  fi
...
访问：
  WebUI  : http://$(hostname -I 2>/dev/null | awk '{print $1}' | head -1):${WEBUI_PORT}
           登录密码见 .env 的 HERMES_WEBUI_PASSWORD（部署后务必修改）
...
控制脚本：
  /opt/hermes/hermesctl.sh          # 数字菜单（模型切换 [17]–[20] 在最后；默认 [17] xiaoyi-gpt-6-astra）
  /opt/hermes/hermesctl.sh {start|stop|restart|status|mcp|switch|test}
  # switch 示例: xiaoyi-gpt-6-astra（默认）| xiaoyi-claude-opus-5-5 | atlascloud-grok-4.3 | atlascloud-grok-4.6

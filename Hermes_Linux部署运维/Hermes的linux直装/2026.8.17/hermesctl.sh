...
切换: switch <provider> [--model m] [--url u]
      provider: xiaoyi-gpt |
                kuaipao-gpt-5.6-sol |
                atlascloud-grok-4.3 | atlascloud-grok-4.5
      （默认 xiaoyi-gpt）
...
show_menu(){
  ...
  echo
  echo -e "${G}  切换主模型（默认: xiaoyi → gpt-5.6-sol）${NC}"
  echo "  [17] xiaoyi                    → gpt-5.6-sol  (默认)"
  echo "  [18] kuaipao-gpt-5.6-sol       → gpt-5.6-sol"
  echo "  [19] atlascloud-grok-4.3        → xai/grok-4.3  (同 atlas API)"
  echo "  [20] atlascloud-grok-4.5        → xai/grok-4.5  (复用 atlas key)"
  echo
  echo "  [0]  退出"
  echo
}
...
menu_loop(){
  ...
  while true; do
    ...
    case "$choice" in
      ...
      17) switch_provider xiaoyi; pause ;;
      18) switch_provider kuaipao-gpt-5.6-sol; pause ;;
      19) switch_provider atlascloud-grok-4.3; pause ;;
      20) switch_provider atlascloud-grok-4.5; pause ;;

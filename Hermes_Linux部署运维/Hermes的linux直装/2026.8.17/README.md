...
| 环境变量 | 渠道 | 用途 |
|---|---|---|
| `HERMES_ATLASCLOUD_KEY` | AtlasCloud | Grok-4.5 / Grok-4.3（回退）|
| `HERMES_KUAIPAO_GPT_KEY` | kuaipao.ai | GPT-5.6-sol |
| `HERMES_XIAOYI_KEY` | xiaoyiapi.xyz | GPT-5.6-sol（xiaoyi 渠道，默认 provider）|
...
通过 `hermesctl.sh` 数字菜单切换（默认 `xiaoyi → gpt-5.6-sol`）：

| 菜单 | Provider | 模型 |
|---|---|---|
| `[17]` | xiaoyi | gpt-5.6-sol（当前生产默认）|
| `[18]` | kuaipao-gpt-5.6-sol | gpt-5.6-sol |
| `[19]` | atlascloud-grok-4.3 | xai/grok-4.3 |
| `[20]` | atlascloud-grok-4.5 | xai/grok-4.5 |

命令行切换：
```bash
/opt/hermes/hermesctl.sh switch xiaoyi

...
step_env(){
  ...
  if [[ -f "${HERMES_DATA}/.env" ]]; then
    ...
    for key in HERMES_KUAIPAO_GPT_KEY HERMES_XIAOYI_KEY; do
...
HERMES_KUAIPAO_GPT_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
HERMES_XIAOYI_KEY=sk-RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
...
# 若当前主模型指向未知 provider，则回退到默认 xiaoyi provider。
m = re.search(r'^  provider:\s*["\']?([^"\'\n]+)', t2, re.M)
known = {"xiaoyi", "kuaipao-gpt", "atlascloud", "atlascloud-grok-4.3", "atlascloud-grok-4.5"}
if m and m.group(1).strip() not in known:
    t2 = re.sub(r'^  default:.*$', '  default: "gpt-5.6-sol"', t2, count=1, flags=re.M)
    t2 = re.sub(r'^  provider:.*$', '  provider: "xiaoyi"', t2, count=1, flags=re.M)
    t2 = re.sub(r'^  base_url:.*$', '  base_url: "https://xiaoyiapi.xyz/v1"', t2, count=1, flags=re.M)
# 回退链中保留 Atlas Grok 4.3 与 Atlas Grok 4.5 各一条（新顺序：4.3 在前）。
fallback_entry = """  - provider: atlascloud-grok-4.3
    model: xai/grok-4.3
    key_env: HERMES_ATLASCLOUD_KEY
  - provider: atlascloud
    model: xai/grok-4.5
...
# 补齐 xiaoyi provider（hermesctl.sh switch xiaoyi 依赖此块；旧版 config.yaml 可能缺失）。
if not re.search(r"^  xiaoyi:\n", t2, flags=re.M):
    xiaoyi_block = (
        "  xiaoyi:\n"
...
控制脚本：
  ...
  # switch 示例: xiaoyi-gpt（默认）| kuaipao-gpt-5.6-sol | atlascloud-grok-4.3 | atlascloud-grok-4.5
